...
--> statement-breakpoint
CREATE UNIQUE INDEX `artifact_version_unique` ON `artifact_versions` (`artifact_id`,`version`);--> statement-breakpoint
CREATE TABLE `artifacts` (
	...
	`archived_at` integer,
	`deleted_at` integer,
	CONSTRAINT "type_check" CHECK("artifacts"."type" in ('spec', 'html'))
);
--> statement-breakpoint
CREATE INDEX `artifact_deletedAt_updatedAt_id_idx` ON `artifacts` (`deleted_at`,`updated_at`,`id`);--> statement-breakpoint
CREATE INDEX `artifact_deletedAt_createdAt_id_idx` ON `artifacts` (`deleted_at`,`created_at`,`id`);--> statement-breakpoint
